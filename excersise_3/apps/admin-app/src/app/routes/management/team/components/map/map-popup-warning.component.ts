...
@Component({
  selector: 'app-marker-popup-warning',
  templateUrl: './map-popup-warning.component.html',
  imports: [DatePipe, HlmSpinnerImports, CommonModule],
  standalone: true,
})
export class MarkerPopupWarningComponent {
  ...
  readonly _warning = signal<string | undefined>(undefined);
  @Input({ required: true })
  set warning(value: string) {
    this._warning.set(value);
  }
